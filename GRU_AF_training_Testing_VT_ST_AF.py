...
model = Sequential()
model.add(GRU(64,input_dim=169))  # try using a GRU instead, for fun
...
checkpointer = callbacks.ModelCheckpoint(filepath="logs/GRU1layer/checkpoint-{epoch:02d}.hdf5", verbose=1, save_best_only=True, monitor='loss')
csv_logger=CSVLogger('logs/GRU1layer/GRUtrainanalysis1.csv',separator=',',append=False)
model.fit(X_train, y_train, batch_size=batch_size, nb_epoch=1000, callbacks=[checkpointer,csv_logger])
model.save("logs/GRU1layer/GRU1layer_model.hdf5")

end_time=time.time()
print("Training Time--- %s seconds ---" % (end_time - start_time))

model=load_model('GRUcheckpoint-981.hdf5')

start_time1 = time.time()
y_pred = model.predict(X_test,batch_size=1000)
np.savetxt('./OUTPUTS/GRUexpected.txt', y_test1, fmt='%01d')
np.savetxt('./OUTPUTS/GRUpredicted.txt', y_pred.round(), fmt='%01d')
